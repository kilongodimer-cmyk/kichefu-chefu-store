...
def seed_marketplace_demo_data():
    car_count = _seed_cars()
    phone_count = _seed_phones()
    accessory_count = _seed_accessories()
    estate_count = _seed_real_estate()
    return {
        "cars": car_count,
        "phones": phone_count,
        "accessories": accessory_count,
        "real_estate": estate_count,
    }


def ensure_seeded_data():
    # Seed once when the storefront database is empty in production.
    if Car.objects.exists() or Phone.objects.exists() or RealEstate.objects.exists():
        return False
